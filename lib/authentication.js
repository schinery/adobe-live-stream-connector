...
Authentication.prototype.getToken = function(callback) {
  ...
  requestToken(function onResponse(err, data) {
    ...
    token.set(authResult.access_token);
...
// Invalidates the current token, presumably because it didn't authorize the
// request.
//
Authentication.prototype.removeToken = function() {
  token.clean();
};
