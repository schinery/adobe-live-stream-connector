...
LiveStream.prototype.connect = function() {
  var self = this;
  var options = self.buildRequestOptions();

  self.req = https.request(options, function onResponse(res) {
    ...
    if(isSuccessful) {
      res.pipe(zlib.createGunzip())
        .pipe(es.split())
        .pipe(es.parse())
        .pipe(es.map(function onMap(data) {
  ...
  self.req.on('close', function onClose() {
    self.request = null;
    self.callback('Stream disconnected. Will attempt reconnect.');
  });

  self.req.on('error', function onError(error) {
    self.request = null;
...
// Attempt to reconnect the Adobe Live Stream.
//
LiveStream.prototype.reconnect = function() {
  this.die();
  this.connect();
};
